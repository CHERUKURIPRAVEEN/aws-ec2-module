...
locals {
  ...
  # local to az_filter to get the availability zones based on the provided availability_zone variable
  az_filter = (var.availability_zone == null ? data.aws_availability_zones.this.names : ["${var.region}${var.availability_zone}"])
